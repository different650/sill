...
function productControl() {
    var $img = $(".detailContainer div:first-of-type aside figure img");
    var baseSrc = $img.attr("src"); // 기본 이미지

    // 초기 이미지: 첫 번째 컬러 라벨
    var $firstColor = $(".color [class*='colorList'] li label").first().text();
    $img.attr("src", baseSrc.replace(".png", "_" + $firstColor + ".png"));

    // 컬러 클릭 이벤트
    $(".color [class*='colorList'] li label").click(function() {
        var selectedColor = $(this).text(); // 클릭한 컬러
        $img.attr("src", baseSrc.replace(".png", "_" + selectedColor + ".png"));

        // active 클래스 처리
        $(this).closest("[class*='colorList']").find("label").removeClass("active");
